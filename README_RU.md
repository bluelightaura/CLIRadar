...
## Конфигурация против каталога

Обход отвечает на вопрос «что этой железке можно сказать»; текущая
конфигурация — «что ей уже сказали». Перед обходом CLIRadar одной командой
(только чтение) снимает конфигурацию и сверяет каждую её строку с каталогом:

```yaml
discovery:
  config_commands:            # пробуются по порядку; берётся первый ответивший
    - display current-configuration
    - show running-config
```

Дамп разбирается структурно — отступы и строки-разделители (`#` на VRP, `!` в
других диалектах) восстанавливают дерево режимов, поэтому платформа, которую
CLIRadar никогда не видел, всё равно даёт дерево. Каждая строка ищется в
каталоге под каждым путём входа, где обход реально стоял; номера экземпляров
сворачиваются, так что `interface 10GE1/0/24` находится по каталогу, снятому
через `interface 10GE1/0/1`.

Главный результат — расхождение. Строка конфигурации, которой нет в каталоге,
означает, что устройство исполняет команду, которую обход не нашёл: это
единственное доказательство неполноты каталога, не требующее второй железки
для сравнения. Такие строки попадают в каталог
(`configuration.missing_from_catalog`) и в HTML-отчёт, причём значения
свёрнуты в `<value>`: какая команда пропущена — свойство платформы, а адрес,
которым она настроена, — свойство вашей сети.

Артефакта два, и они разделены сознательно:

- сводка покрытия в каталоге и отчёте — только «скелеты» команд, можно делиться;
- `output/config_tree.yml` — полное разобранное дерево с реальными значениями,
  каждая необъяснённая строка помечена на месте. Файл описывает живую сеть,
  пишется с приватными правами и не предназначен для публикации. Пароли, ключи
  и community вычищаются до записи.

Чтение дампа идёт со своим бюджетом времени (`device.capture_timeout`, по
умолчанию 120 с): конфигурация листается пейджером и на порядки больше ответа
справки. `config_commands: []` полностью выключает этот шаг.
